...
def train_einet(structure, dataset_name, einet, train_x, train_labels, valid_x, valid_labels, test_x, test_labels,
	...
	if is_adv:
		...
		valid_dataset = fetch_adv_data(einet, dataset_name, valid_x, valid_labels, adv_type=VALID_DATASET)
	else:
...
def fetch_adv_data(einet, dataset_name, inputs, labels, adv_type=TRAIN_DATASET):
	attack, ADV_DATA_DIRECTORY = None, None
	if dataset_name == MNIST or dataset_name == BINARY_MNIST:
		attack = sparsefool_attack
		ADV_DATA_DIRECTORY = "data/{}/augmented/sparsefool".format(dataset_name)
	elif dataset_name in DEBD_DATASETS:
		attack = sparsefool_attack
		ADV_DATA_DIRECTORY = "data/DEBD/datasets/{}/augmented/sparsefool/{}".format(dataset_name,
																				BINARY_DEBD_HAMMING_THRESHOLD)

	test_file_path = os.path.join(ADV_DATA_DIRECTORY, "{}.pt".format(adv_type))
