...
def compute_cluster_idx(data, cluster_means):
	...
	for k in range(len(data)):
		...
		cluster_idx[k] = np.argmin(
			np.sum((cluster_means.reshape(-1, height * width * 3) - img.reshape(1, height * width * 3)) ** 2, 1))
	return cluster_idx
...
def reload_einet(cur_einet, model_file):
	...
	if ARGS.nn:
		param_update_list.append({'params': load_einet.mnist_net.parameters(), 'lr': 3e-4})
	else:
		param_update_list.append({'params': load_einet.mnist_net.parameters(), 'lr': 1e-3, 'weight_decay': 1e-4})
...
def train(einet, mean, train_x, valid_x, test_x, result_path):
	...
	for epoch_count in range(num_epochs):
		...
		if epoch_count % 10 == 0:
			...
			for h in range(5):
				for w in range(5):
					img[h * (height + 10):h * (height + 10) + height, w * (width + 10):w * (width + 10) + width,
					:] = samples[h * 5 + w, :]
		...
		if epoch_count and epoch_count % 40 == 0:
			...
			with torch.no_grad():
				...
				mu2 = params[..., 0:3] ** 2
				params[..., 3:] -= mu2
				params[..., 3:] = torch.clamp(params[..., 3:], exponential_family_args['min_var'],
